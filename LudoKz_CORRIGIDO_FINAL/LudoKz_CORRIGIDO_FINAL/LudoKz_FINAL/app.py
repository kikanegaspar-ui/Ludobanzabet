"""app.py — LudoKz Backend v11
CORREÇÕES v11:
1. BUG CRÍTICO sala não arranca: api_join tinha _collect_reservations
   e r.start() DENTRO do lock _rooms_lk → deadlock → jogo nunca começava.
   Corrigido: add_player dentro do lock, start/collect FORA do lock.
2. BUG dado cola: api_roll devolve sempre o dice real lançado (1-6)
   mesmo quando state.dice já foi reposto a 0 por falta de jogadas.
3. max_players aceita só [2, 4] — removida opção 3.
4. _reserve e _collect_reservations ignoram UIDs negativos (admin/bot).
"""
...
def _reserve(rid: str, uid: int, amount: float) -> bool:
    uid = int(uid)
    if uid < 0:
        return True   # admin/bot — sem saldo real
    ...
    if r:
        for p in r.players:
            ...
            push(p["user_id"], "balance_update", {
                ...
                "msg": "⏰ A sala expirou. Reserva libertada."
            })
...
def _finish_game(rid: str):
    ...
    add_tx(int(winner_id), "prize", prize, f"Prémio vitória — sala {rid}")

    wu = get_user(int(winner_id))
    push(winner_id, "game_over", {
        "won": True, "prize": prize,
        "balance": float(wu["balance"]) if wu else 0,
        "winner_id": winner_id,
    })
    for lid in loser_ids:
        ...
        push(lid, "game_over", {
            "won": False, "prize": 0,
            "balance": float(lu["balance"]) if lu else 0,
            "winner_id": winner_id,
        })
    with _rooms_lk:
        _rooms.pop(rid, None)
...
def api_leave():
    ...
    if not r.started:
        ...
        with _rooms_lk:
            if rid in _rooms:
                _rooms[rid].players = [p for p in r.players
                                       if str(p["user_id"]) != uid_str]
                if not _rooms[rid].players:
                    _rooms.pop(rid, None)
                    _release_reservation(rid)
        return jsonify({"ok": True, "msg": "Saíste. Reserva libertada."})
    ...
    if remaining:
        ...
        credit_prize(int(winner_id), [int(l) for l in loser_ids],
                     r.bet, prize, r.round, rid)
        add_tx(int(winner_id), "prize", prize,
               f"Prémio — adversário abandonou sala {rid}")
...
def api_otp_send():
    ...
    if not ok: return jsonify({"error": "Falha ao enviar SMS."}), 500
    op = operadora(phone)
    return jsonify({"ok": True, "phone": phone, "operadora": op,
                    "msg": f"Código enviado para {phone} ({op}). Válido 2 min."})
...
def api_register():
    ...
    if not age_ok:
        return jsonify({"error": "Confirma que tens 18+ anos."}), 400
    if not terms_ok:
        return jsonify({"error": "Aceita os Termos e Condições."}), 400
    otp_ok = (session.get("otp_ok_register") == phone or
              bool(d.get("phone_verified") or d.get("otp")))
    if not otp_ok:
        return jsonify({"error": "Verifica o número por SMS primeiro."}), 400
    try:
...
def api_reset_req():
    ...
    if not u:
        return jsonify({"ok": True, "msg": "Se o número existir, receberás SMS."})
    code = criar_otp(phone, "reset")
    ok, msg = enviar_sms_simulado(phone, code, u.get("name", "utilizador"))
    if not ok:
        return jsonify({"error": "Falha ao enviar SMS."}), 500
    push_admin("password_reset", {"phone": phone, "code": code})
    return jsonify({"ok": True, "msg": f"Código enviado para {phone}."})
...
def api_dep_req():
    ...
    push_admin("new_deposit", {"deposit_id": did, "amount": amt,
                               "ref": ref, "payer": payer,
                               "pending_count": len(deps)})
...
def api_create():
    ...
    if max_p not in [2, 4]:  return jsonify({"error": "Número de jogadores inválido."}), 400
...
# ══════════════════════════════════════════════════════════════
#  ENTRAR NA SALA — v11 CORRIGIDO
#  PROBLEMA: add_player + _collect_reservations + r.start()
#  estavam todos dentro de _rooms_lk → deadlock → jogo não arrancava.
#  SOLUÇÃO: só add_player dentro do lock; start/collect fora.
# ══════════════════════════════════════════════════════════════
...
def api_join():
    ...
    # ── FASE 1: validar e adicionar jogador (dentro do lock) ──
    should_start = False
    u = None
    ...
    with _rooms_lk:
        ...
        # Verifica se sala ficou cheia dentro do lock
        if r.player_count() >= r.max_players:
            should_start = True
    ...
    # ── FASE 2: cobrar e arrancar (FORA do lock) ──────────────
    if should_start:
        ...
        ok3, start_err = r.start()
        if ok3:
            state = r.state_dict()
            push_room(rid, "game_started", state)
        else:
            app.logger.error(f"start() falhou sala {rid}: {start_err}")
...
# ══════════════════════════════════════════════════════════════
#  ROLAR DADO — v11 CORRIGIDO
#  Devolve sempre "dice" com o valor REAL lançado (1-6),
#  mesmo que state.dice já seja 0 (turno passou automaticamente).
# ══════════════════════════════════════════════════════════════
...
def api_roll():
    ...
    dice_val, err = r.roll_dice(str(session["uid"]))
    if err: return jsonify({"error": err}), 400
    ...
    # Garante que "dice" tem sempre o valor lançado (1-6)
    return jsonify({**state, "dice": dice_val})
    ...
    if ok:
        ...
        if w:
            push(w["user_id"], "withdrawal_rejected", {
                "amount": float(w["amount"]),
                "msg": "Levantamento rejeitado. Saldo devolvido."
            })
    return jsonify({"ok": ok})
...
def adm_stats():
    ...
    return jsonify({"stats": {
        ...
        "games": games,
        "pending_deposits": pdeps, "pending_withdrawals": pwits,
    }})
...
def adm_add_balance():
    ...
    if u: push(uid, "balance_update", {"balance": float(u["balance"]),
                                        "msg": f"Saldo ajustado: +{amount:,.0f} Kz"})
    return jsonify({"ok": True})
...
def adm_login_as_player():
    ...
    conn.commit(); cur.close(); conn.close()
    session["uid"] = uid; session["is_admin"] = True
    return jsonify({"ok": True, "user": {"id": uid, "name": name,
                                          "balance": 999999999,
                                          "phone": ADMIN_PHONE}})
...
def adm_play_create():
    d = request.json or {}
    rid = _make_rid()
    room = GameManager(rid, 0, int(d.get("max_players", 2)),
                       -999, d.get("name", "Admin").strip())
    with _rooms_lk:
        _rooms[rid] = room
    return jsonify({"ok": True, "room_id": rid})
...
def adm_play_join():
    ...
    if r.player_count() >= r.max_players:
        return jsonify({"error": "Sala cheia."}), 400
    if r.started:
        return jsonify({"error": "Já iniciado."}), 400
    ok, err = r.add_player(-(1000 + int(d.get("player_idx", 1))),
                           d.get("name", "Jogador").strip())
    if not ok: return jsonify({"error": err}), 400
...
def adm_play_roll():
    ...
    r   = _get_room(rid)
    if not r: return jsonify({"error": "Sala não encontrada."}), 404
    dice, err = r.roll_dice(str(int(d.get("uid", -999))))
    if err: return jsonify({"error": err}), 400
    state = r.state_dict()
    push_room(rid, "game_update", state)
...
def adm_play_move():
    ...
    r   = _get_room(rid)
    if not r: return jsonify({"error": "Sala não encontrada."}), 404
    state, err = r.move_piece(str(int(d.get("uid", -999))),
                               int(d.get("piece", 0)))
    if err: return jsonify({"error": err}), 400
    push_room(rid, "game_update", state)
    return jsonify(state)
...
def adm_play_movable():
    ...
    r   = _get_room(rid)
    if not r: return jsonify({"movable": []})
    return jsonify({"movable": r.get_movable(str(int(d.get("uid", -999)))),
                    "dice": r.dice})
...
def adm_promo_create():
    ...
    create_promo(code, amount, int(d.get("max_uses", 100)), d.get("expires", ""))
    return jsonify({"ok": True})
...
def adm_ticket_reply(tid):
    ...
    if ticket:
        push(ticket["user_id"], "support_reply", {
            ...
            "msg": f"Suporte respondeu: {reply[:80]}"
        })
    return jsonify({"ok": True})
...
def api_promo_use():
    ...
    return jsonify({"ok": True, "amount": result, "msg": "Bónus aplicado!"})
...
def api_leaderboard():
    ...
    cur.execute("""
        SELECT name, wins, games_played, total_earned,
               ...
               THEN ROUND(wins*100.0/games_played,1) ELSE 0 END AS win_rate
...
if __name__ == "__main__":
    ...
    print("=" * 52)
    print("  LudoKz v11 — Servidor iniciado")
